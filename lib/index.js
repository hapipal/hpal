...
exports.start = async (options) => {
    ...
    const allArgs = ((commandAndPart === 'run') ? options.argv : args._) || [];

    const metaFlags = internals.getMetaFlags(args);

    if (metaFlags.includes('--color') && metaFlags.includes('--no-color')) {
        throw new DisplayError(colors.red('Options [--color] and [--no-color] conflict with each other.'));
    }

    const argsSansMetaFlags = allArgs.filter((arg) => !metaFlags.includes(arg));
    ...
    switch (command) {
        ...
        case 'make': {
            ...
            const place = argsSansMetaFlags[3];
            const name = argsSansMetaFlags[4];
        ...
        case 'docs': {
            ...
            const query = argsSansMetaFlags[3];
            const itemQuery = argsSansMetaFlags[4];
        ...
        case 'run': {
            ...
            const cmd = argsSansMetaFlags[3] && !argsSansMetaFlags[3].startsWith('-') ? argsSansMetaFlags[3] : null;
            const normalizedArgs = allArgs.slice(4);

            if (!cmd && !list) {
                throw new DisplayError(colors.red('You must specify a command to run.'));
            }

            return await Commands.run(cwd, list, cmd, normalizedArgs, ctx);
        }
...
Commands:
  ...
  ${colors.green('hpal docs')}[:<package-name>] [--ver x.y.z|ref] <docs-section> [<config-item>]
    ${colors.yellow('e.g.')} hpal docs --ver 17.2.0 h.continue

  ${colors.green('hpal run')} [--list] <cmd> [<cmd-options>]
    ${colors.yellow('e.g.')} hpal run plugin-name:command-name
`;

internals.getMetaFlags = (args) => {

    if (args instanceof Error) {
        return [];
    }

    return Object.entries(internals.definition).filter(([, definitionObj]) => {

        return definitionObj.description && definitionObj.description.startsWith('[*]');
    })
        .filter(([flagName]) => !!args[flagName])
