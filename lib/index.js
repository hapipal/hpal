...
exports.start = (options) => {
    ...
    switch (command) {
        ...
        case 'docs': {
            ...
            return internals.commands.docs(cwd, version, query, options)
                .then((docs) => {

                    if (!docs) {
                        throw new DisplayError(`Sorry, couldn't find documentation for "${query}".`);
                    }
...
internals.commands = {
    ...
    docs(cwd, version, query, options) {
        ...
        return PkgDir(cwd)
            .then((root) => {
                ...
                return Promise.all([
                    ...
                    internals.getManifest(root).catch((err) => {
            ...
            .then((results) => {
                ...
                const matcher = (section) => {
                    ...
                    return matches.some((match) => {
                        ...
                        const against = (match[0] === '#') ? internals.anchorize(section): section;

                        return ~against.indexOf(match.toLowerCase());
                    });
                };

                return Print.markdownSection(api, matcher);
            });
    },
    new(cwd, dir, options) {
