...
Commands:
  ...
  ${ctx.colors.green('hpal docs')} [--hapi x.y.z] <docs-section> [<config-item>]
    ${ctx.colors.yellow('e.g.')} hpal docs --hapi 16.0.0 reply.continue
`;
        ...
        return PkgDir(cwd)
            .then((root) => {
                ...
                return Promise.all([
                    ...
                    root && internals.getManifest(root, ctx, true).catch((err) => {
...
internals.getManifest = (root, ctx, amendmentsNotRequired) => {
    ...
    return internals.getAmendmentFile(root, ctx)
        ...
        .catch((err) => {
            ...
            const HauteCouture = internals.getHauteCouture(root, ctx);
            const manifest = HauteCouture.manifest.create([], true);
            manifest.file = null;
