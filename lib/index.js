...
exports.start = (options) => {
    ...
    switch (command) {
        ...
        case 'docs': {
            ...
            return internals.commands.docs(cwd, version, place)
                .then((docs) => {

                    if (!docs) {
                        throw new DisplayError(`Sorry, couldn't find documentation for "${place}".`);
                    }
    ...
    docs(cwd, version, place) {
      ...
      return PkgDir(cwd)
          ...
          .then((results) => {
              ...
              return Print.markdownSection(api, (section) => ~section.indexOf(`server.${item.method}(`));
          });
    },
    new(cwd, dir, options) {

        return internals.ensureGit()
            .then(() => {
            ...
            .then(() => {
                ...
                return internals.exec(`git checkout --orphan master`, { cwd: dir });
            })
            ...
            .then((pkg) => {
                ...
                return Pify(Fs.writeFile)(Path.join(dir, 'package.json'), JSON.stringify(pkg));
            })
            .then(() => {

                return internals.spawn('npm', ['init'], { cwd: dir, stdio: [options.in, options.out, options.err] });
            })
            .then(() => {
            ...
            .then((pkg) => {
                ...
                return Pify(Fs.writeFile)(Path.join(dir, 'README.md'), `# ${pkg.name}`);
            })
            .then(() => {
...
internals.getHapiVersion = (root) => {
    ...
    try {
        ...
        if (err.code === 'MODULE_NOT_FOUND') {
            throw new DisplayError(`Couldn't find the hapi package in this project.  It may just need to be installed.`);
        }
...
internals.getHapiAPI = (version) => {
    ...
    return Wreck.get(`https://raw.githubusercontent.com/hapijs/hapi/v${version}/API.md`)
        .then((response) => response.payload.toString())
        .catch((err) => {

            throw new DisplayError(`Could not fetch the hapi API: ${err.message}`);
        });
};

internals.ensureGit = () => {

    return internals.exec('git --version')
        .catch((err) => {

            throw new DisplayError(`To use this command you must have git installed and in your PATH: ${err.message}`)
        });
};

internals.exec = Pify(ChildProcess.exec);

internals.spawn = (cmd, args, options) => {

    return new Promise((resolve, reject) => {

        ChildProcess.spawn(cmd, args, options).once('close', (code) => {
