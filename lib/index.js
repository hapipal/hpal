    ...
    switch (command) {
        case 'make': {
            ...
            if (args.asDir && args.asFile) {
                throw new Error('Options [-d, --asDir] and [-f, --asFile] conflict with each other.');
            }
    ...
    make(cwd, place, name, asDir) {
        ...
        return PkgDir(cwd)
            ...
            .then((results) => {
                ...
                if (!item.list && name) {
                    throw new Error(`${item.place} should be declared once and not in a list, so we can't use name "${name}".`);
                }
                ...
                const filepath = (() => {
                    ...
                    if ((item.list && asDir === false) || (!item.list && asDir === null)) {
