...
exports.start = (options) => {
    ...
    switch (command) {
        case 'make': {
            ...
            if (!place) {
                throw new DisplayError(colors.red('Ah, but what to make?  You must specify a haute-couture item.'));
            }
    ...
    make(cwd, place, name, asDir, ctx) {
        ...
        return PkgDir(cwd)
            .then((root) => {
                ...
                if (!root) {
                    throw new DisplayError(ctx.colors.red('You don\'t seem to be in a project.'));
                }
