...
exports.start = (options) => {
    ...
    switch (command) {
        case 'make': {
            ...
            return internals.commands.make(cwd, place, name, asDir, colors)
                .then((filepath) => {
        ...
        case 'docs': {
            ...
            return internals.commands.docs(cwd, version, query, itemQuery, options, colors)
                .then((docs) => {
        ...
        case 'new': {
            ...
            return internals.commands.new(cwd, dir, options, colors)
                .then(() => {
...
internals.commands = {
    make(cwd, place, name, asDir, colors) {

        return PkgDir(cwd)
            .then((root) => internals.getManifest(root, colors))
            .then((manifest) => {
                ...
                if (!item) {
                    throw new DisplayError(colors.red(`We don't know anything about "${place}".  Try one of: ${manifest.map((m) => m.place).join(', ')}.`));
                }

                if (!item.list && name) {
                    throw new DisplayError(colors.red(`${item.place} should be declared once and not in a list, so we can't use name "${name}".`));
                }
                ...
                return internals.write(filepath, contents, colors).then(() => filepath);
            });
    },
    docs(cwd, version, query, itemQuery, options, colors) {
                ...
                return Promise.all([
                    internals.getHapiAPI(version).catch((err) => {
                        ...
                        if (err.isBoom && err.output.statusCode === 404) {
                            throw new DisplayError(colors.red('Couldn\'t find docs for that version of hapi. Are you sure it\'s a published version?'));
                        }

                        throw new DisplayError(colors.red(`Could not fetch the hapi docs: ${err.message}`));
                    }),
                    internals.getManifest(root, colors).catch((ignoreErr) => {

                        options.err.write(colors.yellow('(Just so you know, we couldn\'t load a haute-couture manifest, so we can\'t search the hapi docs quite as intelligently as usual.)\n\n'));
                    })
                ]);
            })
            .then((results) => {
    ...
    new(cwd, dir, options, colors) {

        return internals.ensureGitAndNpm(colors)
            .then(() => {
...
internals.getManifest = (root, colors) => {

    return internals.getAmendmentFile(root, colors)
        .then((amendmentFile) => {

            const HauteCouture = internals.getHauteCouture(root, colors);
...
internals.getAmendmentFile = (root, colors) => {
    ...
    return Deglob('**/.hc.js', { cwd: root, ignore: 'node_modules/**' })
        .then((amendmentFiles) => {
            ...
            if (amendmentFiles.length > 1) {
                throw new DisplayError(colors.red(`It's ambiguous which directory containing a .hc.js file to use: ${amendmentFiles.join(', ')}`));
            }
...
internals.ensureGitAndNpm = (colors) => {
...
internals.write = (filepath, contents, colors) => {
    ...
    return Mkdirp(Path.dirname(filepath))
        ...
        .catch((err) => {
            ...
            if (err.code === 'EEXIST') {
                throw new DisplayError(colors.red(`The file ${filepath} already exists.`));
            }
